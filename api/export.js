...
export default async function handler(req, res) {
  ...
  if (req.query.senha !== SENHA) return res.status(401).json({ erro: "Senha inválida. Acesso negado." });

  try {
    // LRANGE 0 -1: busca todos os itens da lista do Redis
    const itens = await kv.lrange("quadros_eletricos", 0, -1);

    // Cada item foi salvo como string JSON — faz o parse de cada um
